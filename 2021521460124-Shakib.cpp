...
#define ANIMATION            13
#define DRAW_MECH            14
#define DRAW_ENVIRO          15
#define MOVE_LIGHT           16
#define TEXTID               17
...
#ifdef LIGHT
// Material definitions for changing the color of the robot box
...
// Material definitions for changing the wall color
...
// Material definitions for changing the platform color
...
void Heel1Add(void) { heel1 = (heel1 + 3) % 360; }
void Heel1Subtract(void) { heel1 = (heel1 - 3) % 360; }
void Heel2Add(void) { heel2 = (heel2 + 3) % 360; }
void Heel2Subtract(void) { heel2 = (heel2 - 3) % 360; }
void Ankle1Add(void) { ankle1 = (ankle1 + 3) % 360; }
void Ankle1Subtract(void) { ankle1 = (ankle1 - 3) % 360; }
void Ankle2Add(void) { ankle2 = (ankle2 + 3) % 360; }
void Ankle2Subtract(void) { ankle2 = (ankle2 - 3) % 360; }
void RotateAdd(void) { pivot = (pivot + 3) % 360; }
void RotateSubtract(void) { pivot = (pivot - 10) % 360; }
void MechTiltSubtract(void) { tilt = (tilt - 10) % 360; }
void MechTiltAdd(void) { tilt = (tilt + 10) % 360; }
void elbow1Add(void) { elbow1 = (elbow1 + 2) % 360; }
void elbow1Subtract(void) { elbow1 = (elbow1 - 2) % 360; }
void elbow2Add(void) { elbow2 = (elbow2 + 2) % 360; }
void elbow2Subtract(void) { elbow2 = (elbow2 - 2) % 360; }
void shoulder1Add(void) { shoulder1 = (shoulder1 + 5) % 360; }
void shoulder1Subtract(void) { shoulder1 = (shoulder1 - 5) % 360; }
void shoulder2Add(void) { shoulder2 = (shoulder2 + 5) % 360; }
void shoulder2Subtract(void) { shoulder2 = (shoulder2 - 5) % 360; }
void shoulder3Add(void) { shoulder3 = (shoulder3 + 5) % 360; }
void shoulder3Subtract(void) { shoulder3 = (shoulder3 - 5) % 360; }
void shoulder4Add(void) { shoulder4 = (shoulder4 + 5) % 360; }
void shoulder4Subtract(void) { shoulder4 = (shoulder4 - 5) % 360; }
void lat1Raise(void) { lat1 = (lat1 + 5) % 360; }
void lat1Lower(void) { lat1 = (lat1 - 5) % 360; }
void lat2Raise(void) { lat2 = (lat2 + 5) % 360; }
void lat2Lower(void) { lat2 = (lat2 - 5) % 360; }
void FireCannon(void) { fire = (fire + 20) % 360; }
void RaiseLeg1Forward(void) { hip11 = (hip11 + 3) % 360; }
void LowerLeg1Backwards(void) { hip11 = (hip11 - 3) % 360; }
void RaiseLeg1Outwards(void) { hip12 = (hip12 + 10) % 360; }
void LowerLeg1Inwards(void) { hip12 = (hip12 - 10) % 360; }
void RaiseLeg2Forward(void) { hip21 = (hip21 + 3) % 360; }
void LowerLeg2Backwards(void) { hip21 = (hip21 - 3) % 360; }
void RaiseLeg2Outwards(void) { hip22 = (hip22 + 10) % 360; }
void LowerLeg2Inwards(void) { hip22 = (hip22 - 10) % 360; }
...
void ForeArm(char solid) {
  ...
  SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
  glColor3f(0.0, 1.0, 1.0);  // Forearm light green
  for (i = 0; i < 5; i++) {
...
void UpperLeg(char solid) {
  ...
  SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
  glColor3f(0.0, 1.0, 0.0);  // Above the leg joint and below the foreleg
  glRotatef(-90.0, 1.0, 0.0, 0.0);
  ...
  SetMaterial(mat_specular2, mat_ambient2, mat_diffuse2, mat_shininess2);
#endif
  glColor3f(1.0, 0.0, 0.0);  // Leg joints between foreleg and leg
...
void LowerLeg(char solid) {
  ...
  for (k = 0.0; k < 2.0; k++) {
    for (l = 0.0; l < 2.0; l++) {
      ...
      SetMaterial(mat_specular, mat_ambient, mat_diffuse, mat_shininess);
#endif
      glColor3f(0.0, 1.0, 0.0);  // Leg and foot joints color
      Box(1.0, 0.5, 1.0, solid);
...
void RocketPod(char solid) {
  ...
  glColor3f(0.0, 1.0, 0.0);  // Rocket pod color
...
void display(void) {
  ...
  glLoadName(TEXTID);
  glColor3f(1, 1, 0);
  DrawTextXY(-2.5, 0.2, 2.0, 0.0015, "Batch 2021");
  DrawTextXY(-2.5, 0.5, 2.0, 0.0015, "CG-Final Project");
  DrawTextXY(2.5, 2.2, -2.0, 0.0025, "100shakibalhasan@gmail.com");
  DrawTextXY(2.5, 2.7, -2.0, 0.0030, "2021521460124-Shakib");
  glFlush();
  glutSwapBuffers();
}
...
int main(int argc, char **argv)
{
  ...
  glutCreateWindow("glutmech: Vulcan");
  myinit();
